...
import { deleteStatus } from "../../redux/contactSlice";

export default function ContactStatuses() {
  const statuses = useSelector((state) => state.contactStatuses || {});
  const contacts = useSelector((state) => state.contacts || []); // Додали контакти для підрахунку
  const dispatch = useDispatch();

  const handleDelete = (statusName) => {
    // eslint-disable-next-line no-restricted-globals
    if (confirm(`Delete status "${statusName}"?`)) {
        ...
        <table className="table table-striped table-hover">
          ...
          <tbody>
            {Object.keys(statuses).map((status, index) => {
              ...
              const realCount = contacts.filter(
